...
class App(ctk.CTk):
    def __init__(self, db):
        ...
        try:
            ...
            try:
                ...
                print("Locale italiano non trovato, usando il default di sistema.")
        ...
        for i, day in enumerate(days):
            label = ctk.CTkLabel(self, text=day, font=ctk.CTkFont(size=16, weight="bold"))
    ...
    def create_calendar_grid(self):
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(self.calendar_frame, label_text="Orari")
        ...
        for i in range((end_hour - self.calendar_start_hour) * 2):
            ...
            for day in range(5):
                cell_frame = ctk.CTkFrame(self.scrollable_frame, border_width=1, fg_color="transparent", corner_radius=0, height=25)
                cell_frame.grid(row=i, column=day+1, padx=0, pady=0, sticky="nsew")
                cell_frame.grid_columnconfigure((0, 1, 2), weight=1)
    ...
    def refresh_events(self):
        ...
        for event in sorted(events, key=lambda x: x['start_time']):
            ...
            while current_dt < end_dt:
                col = current_dt.weekday()
                row = (current_dt.hour - self.calendar_start_hour) * 2 + (1 if current_dt.minute >= 30 else 0)
                if not (0 <= row < len(self.calendar_cells)):
                    current_dt += timedelta(minutes=30)
                    continue
                sub_col = 0
                while (row, col, sub_col) in cell_occupancy:
                    sub_col += 1
                if sub_col > 2:
                    current_dt += timedelta(minutes=30)
                    continue
                cell_occupancy[(row, col, sub_col)] = event['id']
                event_text = f"{event['name']}"
                event_button = ctk.CTkButton(self.calendar_cells[row][col], text=event_text, fg_color="#c9514a" if event['is_logged'] else "#3b8ed0", command=lambda e=event: self.open_view_event_window(e), font=ctk.CTkFont(size=10))
                event_button.grid(row=0, column=sub_col, padx=1, pady=1, sticky="nsew")
